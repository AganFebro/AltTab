...
namespace {
    ...
    void RemoveStartupFromRegistry() {
        ...
        if (result == ERROR_SUCCESS) {
            RegDeleteValue(hKey, AT_PRODUCT_NAMEW);
            RegCloseKey(hKey);
        }
    }
}
...
wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int /*nCmdShow*/) {
    ...
    if (g_GeneralSettings.IsProcessElevated && !g_GeneralSettings.IsTaskElevated
        && wcsstr(GetCommandLineW(), L"--elevated")) {
    ...
    if (!g_KeyboardHook) {
        AT_LOG_ERROR("Failed to install the low-level keyboard hook; AltTab hotkeys will not work.");
    }
    ...
    while ((bRet = GetMessage(&msg, nullptr, 0, 0)) != 0) {
        if (bRet == -1) {
            // GetMessage failed; abandon the loop rather than dispatch a garbage message.
            AT_LOG_ERROR("GetMessage failed.");
            break;
        }
    ...
    // Un-initialize common platform services.
    UninitializeCOM();
...
LRESULT CALLBACK MainWndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_DRAWITEM:
        if (ThemedMenuSession::HandleDrawItem(reinterpret_cast<DRAWITEMSTRUCT*>(lParam)))
            return TRUE;
        return DefWindowProc(hWnd, message, wParam, lParam);
...
void DeleteNotificationIcon(HWND hWndTrayIcon) {
    // Remove the tray icon so it does not linger as a ghost after exit.
    NOTIFYICONDATA nid = { 0 };
    nid.cbSize = sizeof(NOTIFYICONDATA);
    nid.hWnd = hWndTrayIcon;
    nid.uID = 1;
        ...
        if (wParam == WM_KEYDOWN || wParam == WM_SYSKEYDOWN) {
            if (g_hAltTabWnd == nullptr) {
                ...
                bool showAltTabWindow =
                    (g_Settings.HKAltTabEnabled && !isCtrlPressed) || (g_Settings.HKAltCtrlTabEnabled && isCtrlPressed);
        ...
        if (!checked) {
            ...
            if (isRunAtStartup) {
                ...
                const int result = MessageBoxW(
                    ...
                    L"This will change the AltTab startup task to 'Run with highest privileges' option in Task "
                    L"Scheduler.\n"
    ...
    if (hMenu) {
        ...
        if (hSubMenu) {
            ...
            if (isRunAtStartup) {
                ...
                SetMenuItemInfoW(hSubMenu, ID_TRAYCONTEXTMENU_RUNASADMIN, FALSE, &mii);
            } else {
                MENUITEMINFOW mii = { sizeof(mii) };
                mii.fMask = MIIM_STRING;
...
bool IsRunAtStartup() {
    ...
    HKEY hKey;
    LONG result =
        RegOpenKeyEx(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Run", 0, KEY_READ, &hKey);
...
int GetCurrentYear() {
    ...
    // Convert to local time using the thread-safe variant (std::localtime returns
    // a shared static buffer and can return nullptr).
    std::tm localTime = {};
    if (localtime_s(&localTime, &currentTime) != 0) {
        // Fall back to the build year on failure.
        return AT_VERSION_MAJOR;
    }
