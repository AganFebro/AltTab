...
std::string GetWindowTitleExA(HWND hWnd) {
    ...
    if (length <= 0)
        return std::string();
    std::string ret(static_cast<size_t>(length), '\0');
    const int copied = GetWindowTextA(hWnd, &ret[0], length + 1);
    ret.resize(static_cast<size_t>(copied));
    return ret;
}
...
std::wstring GetWindowTitleExW(HWND hWnd) {
    ...
    if (length <= 0)
        return std::wstring();
    std::wstring ret(static_cast<size_t>(length), L'\0');
    const int copied = GetWindowTextW(hWnd, &ret[0], length + 1);
    ret.resize(static_cast<size_t>(copied));
    return ret;
}
...
std::string ToNarrow(const wchar_t* szBufW) {
    ...
    if (err != 0) {
        throw std::runtime_error("Failed to convert to a multi byte character string.");
    }
...
int GetDPIForWindow(HWND hWnd) {
    HDC hdc = GetDC(hWnd);
    if (!hdc)
        return 96; // Fall back to the standard default DPI.
...
bool CreateAutoStartTask(bool runElevated) {
    ...
    if (FAILED(hr)) {
        ...
        pRootFolder->Release(); // release on both success and failure paths
...
bool CheckSingleInstance(const std::wstring& mutexName) {
    ...
    if (hMutex != nullptr && lastError == ERROR_ALREADY_EXISTS) {
        // Another instance already owns the named mutex.
        CloseHandle(hMutex);
        return false;
    }
    // Either this is the first instance (the mutex is intentionally kept open for
    // the process lifetime) or CreateMutexW failed; allow this instance to run.
    return true;
}
...
void InitGDIPlus() {
    ...
    if (Gdiplus::GdiplusStartup(&g_gdiplusToken, &gdiplusStartupInput, nullptr) != Gdiplus::Ok) {
        g_gdiplusToken = 0;
        AT_LOG_ERROR("GdiplusStartup failed.");
    }
}
